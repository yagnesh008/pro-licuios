...
const DeliveryLogin = () => {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (contact.trim().toLowerCase() !== DELIVERY_EMAIL || password !== DELIVERY_PASSWORD) {
      toast.error("Invalid delivery credentials");
      return;
    }
    login({
      id: "staff_delivery",
      ...
      <div className="w-full max-w-md animate-fade-in-up">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="font-display text-2xl font-bold inline-flex items-center gap-2 justify-center">
            <Bike className="h-6 w-6 text-primary" /> Delivery login
          </h1>
          <p className="text-sm text-muted-foreground mt-1">Pick up & deliver assigned orders</p>
        </div>
        <div className="glass-card rounded-3xl p-7 shadow-elevated">
          <form onSubmit={submit} className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="contact">Rider email</Label>
              <Input id="contact" value={contact} onChange={(e) => setContact(e.target.value)} placeholder="rider@prolicious.in" className="h-11 rounded-xl" />
            </div>
            <div className="space-y-1.5">
            ...
            <Button type="submit" variant="hero" size="lg" className="w-full">Sign in as Rider</Button>
          </form>
          <p className="text-[11px] text-center text-muted-foreground mt-5">Delivery accounts are pre-configured by the platform.</p>
        </div>
      </div>
    </div>
  );
